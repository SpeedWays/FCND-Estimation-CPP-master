...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Mat3x3F Ro = Mat3x3F();
  Ro(0,0) = 1;
  Ro(0,1) = sin(phi) * tan(theta);
  Ro(0,2) = cos(phi) * tan(theta);
  Ro(1,0) = 0;
  Ro(1,1) = cos(phi);
  Ro(1,2) = -sin(phi);
  Ro(2,0) = 0;
  Ro(2,1) = sin(phi) / cos(theta);  
  Ro(2,2) = cos(phi) / cos(theta);  
  
  V3F gyr(gyro.x, gyro.y, gyro.z); 
  V3F angle_dot = Ro * gyr;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) = curState(3) + acc_i.x * dt; 
  predictedState(4) = curState(4) + acc_i.y * dt; 
  predictedState(5) = curState(5) + acc_i.z * dt - CONST_GRAVITY * dt; // change in velocity along the z is a_z * dt by removing the gravity component
